...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate VERA-RL")
...
def main() -> None:
    ...
    vera.load_state_dict(dict(payload.get("vera_state", {})))
    ...
    runner = RolloutRunner(
        ...
        environment=ToyEnvironment(seed=seed + 13),
        policy=policy,
        verifier=ToyVerifier(),
    )
    rollouts = [runner.run_episode(task) for task in eval_tasks]
    metrics = vera.train_step(rollouts, policy=None)
    summary = {
